...
int main(){
    DWORD health_offset[] = {0x2F0};

    char processname[] = "CoDSP.exe" ;
    if (attachprocess(processname)){
        char modulename1[] = "gamex86.dll";

        //get the game's base address
        DWORD gamebaseaddress1 = GetModuleBaseAddress(modulename1,pId);

        //address
        DWORD healt_address = 0x0005C8E4;
        DWORD addresstowrite = GetPointerAddress(gamebaseaddress1,healt_address,1,health_offset);

        while(true)
            wpm<int>(600,addresstowrite);    
    }
